...
server <- function(input, output, session) {
  ...
  output$HCGC_A0 <- renderHighchart({
    ...
    Egresos_GC_GC_anual  <- highchart() %>% 
      ...
      hc_xAxis(categories = Egresos_GC_GC_anual$Ano) %>% 
  ...
  output$HCGC_A2 <- renderHighchart({
    ...
    Egresos_GC_GC_anual_variacion  <- highchart() %>% 
      ...
      hc_tooltip(crosshairs = TRUE, backgroundColor = "#FCFFC5",
                 shared = TRUE, borderWidth = 5) %>% 
      hc_exporting(enabled = TRUE) %>% 
      hc_xAxis(categories = Egresos_GC_GC_anual_variacion$Ano) %>% 
  ...
  output$COG_TOTAL_A_G <- renderHighchart({
    ...
    COG_T_A  <- highchart() %>% 
      ...
      hc_tooltip(crosshairs = TRUE, backgroundColor = "#FCFFC5",
                 shared = TRUE, borderWidth = 5) %>% 
      hc_exporting(enabled = TRUE) %>% 
      hc_xAxis(categories = COG_T_A$Ano) %>% 
      ...
      hc_add_series(name = "Amortizacion", data = COG_T_A$`Amortizacion`) %>%
  ...
  output$COG_TOTAL_M_G_VAR <- renderHighchart({
    ...
    COG_T_A <- mutate(COG_T_A,
                      ...
                      var.Amortizacion = round((`Amortizacion`/lag(`Amortizacion`,n = 12)-1)*100,1),
    ...
    COG_T_A <- mutate(COG_T_A, 
                      ...
                      acum_12.Amortizacion = roll_sum(COG_T_A$`Amortizacion`, 12, align = "right", fill = NA),
                      var.acum.Amortizacion = round((acum_12.Amortizacion/lag(acum_12.Amortizacion,n = 12)-1)*100,1),
    ...
    COG_T_A  <- highchart() %>% 
      ...
      hc_add_series(name = "var.Amortizacion", data = COG_T_A$`var.Amortizacion`) %>% 
      hc_add_series(name = "var.acum.Amortizacion", data = COG_T_A$`var.acum.Amortizacion`) %>%
  ...
  output$ECO2_TOTAL_AG_CLA <- renderHighchart({
    ...
    ECO_T_A  <- highchart() %>% 
      ...
      hc_tooltip(crosshairs = TRUE, backgroundColor = "#FCFFC5",
                 shared = TRUE, borderWidth = 5) %>% 
      hc_exporting(enabled = TRUE) %>% 
      hc_xAxis(categories = ECO_T_A$Ano) %>% 
      ...
      hc_add_series(name = "Adquisicion de activos", data = ECO_T_A$`Adquisicion de activos`) %>%
      hc_add_series(name = "Transferencias de capital", data = ECO_T_A$`Transferencias de capital`) %>%
      hc_add_series(name = "Sumas sin asignacion", data = ECO_T_A$`Sumas sin asignacion`) %>%
      hc_add_series(name = "Adquisicion de valores", data = ECO_T_A$`Adquisicion de valores`) %>%
      hc_add_series(name = "Amortizacion", data = ECO_T_A$`Amortizacion`) %>%
  ...
  output$ECO2_TOTAL_MG_CLA <- renderHighchart({
    ...
    ECO_T_M  <- highchart() %>% 
      ...
      hc_tooltip(crosshairs = TRUE, backgroundColor = "#FCFFC5",
                 shared = TRUE, borderWidth = 5) %>% 
      hc_exporting(enabled = TRUE) %>% 
      hc_xAxis(categories = ECO_T_M$Ano) %>% 
      ...
      hc_add_series(name = "Adquisicion de activos", data = ECO_T_M$`Adquisicion de activos`) %>%
      hc_add_series(name = "Transferencias de capital", data = ECO_T_M$`Transferencias de capital`) %>%
      hc_add_series(name = "Sumas sin asignacion", data = ECO_T_M$`Sumas sin asignacion`) %>%
      hc_add_series(name = "Adquisicion de valores", data = ECO_T_M$`Adquisicion de valores`) %>%
      hc_add_series(name = "Amortizacion", data = ECO_T_M$`Amortizacion`) %>%
  ...
  output$ECO2_TOTAL_MG_VAR <- renderHighchart({
    ...
    ECO_T_M <- mutate(ECO_T_M,
                      ...
                      var.Formacion.Capital = round((`Formacion de capital`/lag(`Formacion de capital`,n = 12)-1)*100,1),
                      var.Transferencias.Capital = round((`Transferencias de capital`/lag(`Transferencias de capital`,n = 12)-1)*100,1),
                      var.Adquisicion.Activos = round((`Adquisicion de activos`/lag(`Adquisicion de activos`,n = 12)-1)*100,1),
                      var.Amortizacion = round((`Amortizacion`/lag(`Amortizacion`,n = 12)-1)*100,1),
                      var.Adquisicion.Valores = round((`Adquisicion de valores`/lag(`Adquisicion de valores`,n = 12)-1)*100,1),
    ...
    ECO_T_M <- mutate(ECO_T_M, 
                      ...
                      acum_12.Formacion.Capital = roll_sum(ECO_T_M$`Formacion de capital`, 12, align = "right", fill = NA),
                      ...
                      var.Adquisicion.Activos = roll_sum(ECO_T_M$`Adquisicion de valores`, 12, align = "right", fill = NA),
                      var.acum.Adquisicion.Activos  = round((var.Adquisicion.Activos/lag(var.Adquisicion.Activos,n = 12)-1)*100,1),
                      
                      var.Amortizacion = roll_sum(ECO_T_M$`Amortizacion`, 12, align = "right", fill = NA),
                      var.acum.Amortizacion  = round((var.Amortizacion/lag(var.Amortizacion,n = 12)-1)*100,1),
                      
                      var.Adquisicion.Valores = roll_sum(ECO_T_M$`Adquisicion de valores`, 12, align = "right", fill = NA),
                      var.acum.Adquisicion.Valores  = round((var.Adquisicion.Valores/lag(var.Adquisicion.Valores,n = 12)-1)*100,1),
    ...
    ECO_T_M  <- highchart() %>% 
      ...
      hc_add_series(name = "var.Amortizacion", data = ECO_T_M$`var.Amortizacion`) %>% 
      hc_add_series(name = "var.acum.Amortizacion", data = ECO_T_M$`var.acum.Amortizacion`) %>%
      
      hc_add_series(name = "var.Adquisicion.Valores", data = ECO_T_M$`var.Adquisicion.Valores`) %>% 
      hc_add_series(name = "var.acum.Adquisicion.Valores", data = ECO_T_M$`var.acum.Adquisicion.Valores`) %>%
  ...
  output$COG_TOTAL_M <- renderHighchart({
    ...
    COG_T_M  <- highchart() %>% 
      ...
      hc_add_series(name = "Amortizacion", data = COG_T_M$`Amortizacion`) %>%
  ...
  output$COG_TOTAL_VAR <- renderHighchart({
    ...
    COG_T_M <- mutate(COG_T_M, 
                      ...
                      acum_12.Amortizacion = roll_sum(COG_T_M$`Amortizacion`, 12, align = "right", fill = NA),
                      var.acum.Amortizacion = round((acum_12.Amortizacion/lag(acum_12.Amortizacion,n = 12)-1)*100,1),
    ...
    COG_T_M  <- highchart() %>% 
      ...
      hc_add_series(name = "var.Amortizacion", data = COG_T_M$`var.Amortizacion`) %>% 
      hc_add_series(name = "var.acum.Amortizacion", data = COG_T_M$`var.acum.Amortizacion`) %>%
  ...
  output$clasi_COG_ANUAL_TAM = renderHighchart ({
    ...
    COG_AT  <- highchart() %>% 
      ...
      hc_tooltip(crosshairs = TRUE, backgroundColor = "#FCFFC5",
                 shared = TRUE, borderWidth = 5) %>% 
      hc_exporting(enabled = TRUE) %>% 
      hc_xAxis(categories = COG_AT$Ano) %>% 
      ...
      hc_add_series(name = "Amortizacion", data = COG_AT$`Amortizacion`) %>%
 ...
 #     )
    ...
    COG_AM  <- highchart() %>% 
      ...
      hc_add_series(name = "Amortizacion",              data = COG_AM$`Amortizacion`) %>%
  ...
  output$clasi_COG_VAR_MENSUAL_T = renderHighchart ({
    ...
    COG_AM <- mutate(COG_AM, 
                      ...
                      acum_12.Amortizacion = roll_sum(COG_AM$`Amortizacion`, 12, align = "right", fill = NA),
                      var.acum.Amortizacion = round((acum_12.Amortizacion/lag(acum_12.Amortizacion,n = 12)-1)*100,1),
    ...
    COG_AM  <- highchart() %>% 
      ...
      hc_add_series(name = "var.Amortizacion", data = COG_AM$`var.Amortizacion`) %>% 
      hc_add_series(name = "var.acum.Amortizacion", data = COG_AM$`var.acum.Amortizacion`) %>%
...
#  })
  ...
  output$clasi_COG_titulo_var = renderHighchart ({
    ...
    COG_AM  <- highchart() %>% 
      ...
      hc_tooltip(crosshairs = TRUE, backgroundColor = "#FCFFC5",
                 shared = TRUE, borderWidth = 5) %>% 
      hc_exporting(enabled = TRUE) %>% 
      hc_xAxis(categories = COG_AM$Ano) %>% 
  ...
  output$clasi_ECO2_ANUAL_TAM = renderHighchart ({
    ...
    ECO_AT  <- highchart() %>% 
      ...
      hc_tooltip(crosshairs = TRUE, backgroundColor = "#FCFFC5",
                 shared = TRUE, borderWidth = 5) %>% 
      hc_exporting(enabled = TRUE) %>% 
      hc_xAxis(categories = ECO_AT$Ano) %>% 
      ...
      hc_add_series(name = "Adquisicion de activos", data = ECO_AT$`Adquisicion de activos`) %>%
      hc_add_series(name = "Transferencias de capital", data = ECO_AT$`Transferencias de capital`) %>%
      hc_add_series(name = "Sumas sin asignacion", data = ECO_AT$`Sumas sin asignacion`) %>%
      hc_add_series(name = "Adquisicion de valores", data = ECO_AT$`Adquisicion de valores`) %>%
      hc_add_series(name = "Amortizacion", data = ECO_AT$`Amortizacion`) %>%
  ...
  output$clasi_ECO2_MENSUAL_T = renderHighchart ({
    ...
    ECO_AM  <- highchart() %>% 
      ...
      hc_tooltip(crosshairs = TRUE, backgroundColor = "#FCFFC5",
                 shared = TRUE, borderWidth = 5) %>% 
      hc_exporting(enabled = TRUE) %>% 
      hc_xAxis(categories = ECO_AM$Ano) %>% 
      ...
      hc_add_series(name = "Sumas sin asignacion", data = ECO_AM$`Sumas sin asignacion`) %>%
      hc_add_series(name = "Adquisicion de valores", data = ECO_AM$`Adquisicion de valores`) %>%
      hc_add_series(name = "Amortizacion", data = ECO_AM$`Amortizacion`) %>%
  ...
  output$clasi_ECO2_VAR_MENSUAL_T = renderHighchart ({
    ...
    ECO_AM <- mutate(ECO_AM, 
                     ...
                     acum_12.Formacion.Capital = roll_sum(ECO_AM$`Formacion de capital`, 12, align = "right", fill = NA),
                     var.acum.Formacion.Capital  = round((acum_12.Formacion.Capital/lag(acum_12.Formacion.Capital,n = 12)-1)*100,1),
                     
                     var.Transferencias.Capital = roll_sum(ECO_AM$`Transferencias de capital`, 12, align = "right", fill = NA),
                     var.acum.Transferencias.Capital  = round((var.Transferencias.Capital/lag(var.Transferencias.Capital,n = 12)-1)*100,1),
                     
                     var.Adquisicion.Activos = roll_sum(ECO_AM$`Adquisicion de valores`, 12, align = "right", fill = NA),
                     var.acum.Adquisicion.Activos  = round((var.Adquisicion.Activos/lag(var.Adquisicion.Activos,n = 12)-1)*100,1),
                     
                     var.Amortizacion = roll_sum(ECO_AM$`Amortizacion`, 12, align = "right", fill = NA),
                     var.acum.Amortizacion  = round((var.Amortizacion/lag(var.Amortizacion,n = 12)-1)*100,1),
                     
                     var.Adquisicion.Valores = roll_sum(ECO_AM$`Adquisicion de valores`, 12, align = "right", fill = NA),
                     var.acum.Adquisicion.Valores  = round((var.Adquisicion.Valores/lag(var.Adquisicion.Valores,n = 12)-1)*100,1),
    ...
    ECO_AM  <- highchart() %>% 
      ...
      hc_add_series(name = "var.Amortizacion", data = ECO_AM$`var.Amortizacion`) %>% 
      hc_add_series(name = "var.acum.Amortizacion", data = ECO_AM$`var.acum.Amortizacion`) %>%
      
      hc_add_series(name = "var.Adquisicion.Valores", data = ECO_AM$`var.Adquisicion.Valores`) %>% 
      hc_add_series(name = "var.acum.Adquisicion.Valores", data = ECO_AM$`var.acum.Adquisicion.Valores`) %>%
  ...
  output$index7 <- renderValueBox({
    ...
    valueBox(value= ind1, subtitle=paste0("Monto total ejecutado para el ", Ano_actual, " (en millones)"),
  ...
  output$index3.2 <- renderValueBox({
    ...
    valueBox(value= ind3, subtitle=paste0("Monto total ejecutado para el ", Ano_pasado_1, " (en millones)"),
             icon = icon("money"), color = "purple", width = 3)
  ...
  output$tabla_ind1 <- renderReactable ({
    ...
    c <- spread(Egresos_GC_GC_mensual, Ano, round(Ejecutado,1))
  ...
  output$tabla_ind2 <- renderReactable ({
    ...
    Egresos_GC_GC_mensual <- Egresos_GC_GC_mensual %>% 
      ...
      dplyr::select(Ano, mes.cod, mes, var.Ejecutado)
    
    
    d <- spread(Egresos_GC_GC_mensual, Ano, var.Ejecutado)
  ...
  output$tabla_ind3 <- renderReactable ({
    ...
    Egresos_GC_GC_mensual <- Egresos_GC_GC_mensual %>%
      ...
      dplyr::arrange(Ano,mes.cod) %>%
      dplyr::select(Ano,mes,acum_12) 
  ...
  output$tabla_ind4 <- renderReactable ({
    ...
    Egresos_GC_GC_mensual <- Egresos_GC_GC_mensual %>%
      ...
      dplyr::arrange(Ano,mes.cod) %>%
      dplyr::select(Ano,mes,var.acum_12) 
  ...
  output$Ejecutado1_2 = renderValueBox({
    ...
    valueBox(
      paste0(Gastos_pronosticado,1), paste0("Gasto total proyectado para el ", Ano_actual, " (millones)"), icon = icon("money"),
      color = "blue", width = 4
    )
  })
  ...
  output$Ejecutado2_2 = renderValueBox({
    ...
    valueBox(
      paste0(Gastos_pronosticado), paste0("Gasto total para el ", Ano_actual, " (millones)"), icon = icon("money"),
      color = "blue", width = 4
    )
  })
